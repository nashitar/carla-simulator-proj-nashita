...
class JaywalkingCrosswalkPedestrian(CrosswalkPedestrian):
	...
	def __init__(self, location):
		...
		super(JaywalkingCrosswalkPedestrian, self).__init__(location)

	def path (x1, x2, x3, x4, y1, y2, y3, y4):

		# ***********************
		# pedestrian must stay 
		# outside the width constraints 
		# ***********************
		...
		if ((max(x)-min(x))>(max(y)-min(y))):
			...
			if (left_or_right == 1):
				...
				coordinates.append(CrosswalkPedestrian.transition_point(x3, x4, y3-3, y3-1))
			...
			# if the variable left_or_right is equal to 2 the padestrian is going right
				...
				coordinates.append(CrosswalkPedestrian.transition_point(x3, x4, y4+1, y4+3))
		...
		# else of conditional used to check which direction the crosswalk is positioned towards
			...
			if (left_or_right == 1):
				...
				coordinates.append(CrosswalkPedestrian.transition_point(x3-3, x3-1, y3, y4))
			...
			# if the variable left_or_right is equal to 2 the padestrian is going right
				...
				coordinates.append(CrosswalkPedestrian.transition_point(x4+1, x4+3, y3, y4))
